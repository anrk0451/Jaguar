...
namespace Jaguar.BusinessObject
{
	public partial class FireCheckinBrow : BaseBusiness
	{
		...
		private void DataFilter(string action)
		{
			...
			{
				case "今日登记":
					checkin_ds.ac01Adapter.SelectCommand.CommandText = "select * from ac01 where trunc(ac200) = trunc(sysdate) and status <> '0' order by ac001 desc";
					break;
				case "近三日登记":
					checkin_ds.ac01Adapter.SelectCommand.CommandText = "select * from ac01 where (trunc(sysdate) - trunc(ac200)) <=2 and status <> '0'  order by ac001 desc ";
					break;
				case "一个月内登记":
					checkin_ds.ac01Adapter.SelectCommand.CommandText = "select * from ac01 where (trunc(sysdate) - trunc(ac200)) <=30 and status <> '0'  order by ac001 desc ";
					break;
				case "三个月内登记":
					checkin_ds.ac01Adapter.SelectCommand.CommandText = "select * from ac01 where (trunc(sysdate) - trunc(ac200)) <=90 and status <> '0'  order by ac001 desc";
					break;
			}
