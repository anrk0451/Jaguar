...
namespace Jaguar.BusinessObject
{
	public partial class FireBusiness : BaseBusiness
	{
		...
		public override void Business_Init()
		{
			...
			te_ac070.EditValue = reader["AC070"].ToString();	//减免类型
			te_cash.EditValue = reader["AC299"];				//押金余额
			...
			//减免金额
			gridColumn13.SummaryItem.SummaryType = DevExpress.Data.SummaryItemType.Sum;
			gridColumn13.SummaryItem.DisplayFormat = "{0:N2}";
		}
		...
		private void RefreshPanel()
		{
			...
			//押金余额
			te_cash.EditValue = FireAction.GetCash(AC001);

			//减免预收款
			te_precash.EditValue = FireAction.GetPreCash(AC001);
 
			//减免设置
			te_ac070.EditValue = SqlAssist.ExecuteScalar("select ac070 from ac01 where ac001='" + AC001 + "'");
		...
		{			
			if(FireAction.FireIsSettled(AC001) == "1")			 
				if (XtraMessageBox.Show("逝者已经办理完火化结算,是否继续缴纳押金?","提示",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.No) return;
			Frm_Cash frm_1 = new Frm_Cash();
			frm_1.swapdata["ac001"] = AC001;
			frm_1.swapdata["action"] = "pay";
			if(frm_1.ShowDialog() == DialogResult.OK)			 
				this.RefreshPanel();
		...
		{
			Frm_CashSearch frm_1 = new Frm_CashSearch();
			frm_1.swapdata["ac001"] = AC001;
			frm_1.ShowDialog();
			frm_1.Dispose();
		}
		...
		{
			decimal dec_cash = FireAction.GetCash(AC001);
			if(dec_cash <= 0)
			{
				XtraMessageBox.Show("没有押金可以返还!","提示",MessageBoxButtons.OK,MessageBoxIcon.Information);
				return;
			}
			else if(XtraMessageBox.Show("押金余额尚有" + dec_cash.ToString("##,##0.00") + ",是否继续?","提示",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
			{
				Frm_Cash frm_1 = new Frm_Cash();
				frm_1.swapdata["ac001"] = AC001;
				frm_1.swapdata["action"] = "ret";
				if (frm_1.ShowDialog() == DialogResult.OK)
					this.RefreshPanel();
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void te_ac070_CustomDisplayText(object sender, DevExpress.XtraEditors.Controls.CustomDisplayTextEventArgs e)
		{
			if(e.Value != null)
				e.DisplayText = MiscAction.Mapper_DataDict(e.Value.ToString());
		}
		/// <summary>
		/// 设置减免情况
		/// </summary>
		...
		private void barButtonItem25_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			...
			Frm_SetAvoid frm_1 = new Frm_SetAvoid();			
			frm_1.swapdata["ac001"] = AC001;

			if(frm_1.ShowDialog() == DialogResult.OK)
			{
				string s_ac070 = string.Empty;
				s_ac070 = frm_1.swapdata["ac070"].ToString();
				if(s_ac070 != te_ac070.EditValue.ToString())
				{
					if (AvoidItemIsSettled() && XtraMessageBox.Show("已有减免项目结算完成,结算完成的项目将不受影响!是否继续?","提示",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.No) return;
					if(FireAction.ApplyAvoid(AC001,s_ac070) > 0)
					{
						this.RefreshSalesData();
					}
				}
		...
		/// </summary>
		/// <returns></returns>
		private bool AvoidItemIsSettled()
		{
			string s_sa002 = string.Empty;
			for (int i = 0; i < gridView1.RowCount; i++)
			{
				if(gridView1.GetRowCellValue(i,"SA008").ToString() == "1")
				{
					s_sa002 = gridView1.GetRowCellValue(i, "SA002").ToString();
					if (s_sa002 == "01" || s_sa002 == "02" || s_sa002 == "06" || s_sa002 == "07" || s_sa002 == "08") return true;
				}
			}
			return false;
		}
        ...
        {
			decimal dec_precash = FireAction.GetPreCash(AC001);
            if (!(dec_precash > 0))
            {
				XtraMessageBox.Show("逝者没有减免预收款!","提示",MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
				return;
            }
		...
		private void RefundFilter()
		{
			if (!barToggleSwitchItem1.Checked)
			{
				...
				gridView1.ActiveFilterString = "SA222 == null";
			}
			else
			{
				gridView1.ActiveFilter.Clear();
