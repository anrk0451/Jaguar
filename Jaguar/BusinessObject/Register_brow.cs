...
namespace Jaguar.BusinessObject
{
    public partial class Register_brow : BaseBusiness
    {
		...
		private void barButtonItem3_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			...
			//BaseDialog frm_reg;
			//if (Envior.REG_CHOOSE_MODE == "1")
			//	frm_reg = new Frm_Register_SelfChoose();
			//else
			//	frm_reg = new Frm_Register();

			Frm_Register_SelfChoose frm_reg = new Frm_Register_SelfChoose();
		...
		private void barButtonItem2_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			...
			if (frm_fromFire.ShowDialog() == DialogResult.OK)
			{
				...
				Frm_Register_SelfChoose regform = new Frm_Register_SelfChoose();
		...
		private void barButtonItem17_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			...
			if(rowHandle >= 0)
			{
				if (XtraMessageBox.Show("确认要删除当前的记录吗?","提示",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.No) return;
				string s_rc001 = gridView1.GetRowCellValue(rowHandle, "RC001").ToString();
				if(RegisterAction.RemoveReg(s_rc001) > 0)
				{
					this.RefreshData();
